namespace FfxivStartupCommands
{
    ...
    public class GameClient
    {
        ...
        public unsafe bool GetChatVisible()
        {
            try
            {
                var chatLog = Services.GameGui.GetAddonByName("ChatLog", 1);

                if (!chatLog.IsNull)
                {
                    // Cast to the raw pointer if you need to access the struct directly
                    AtkUnitBase* chatLogPtr = (AtkUnitBase*)chatLog.Address;
                    return chatLogPtr->IsVisible;
                }
            }
            catch (Exception)
            {
